...
export default function ProductSkeleton({ admin = false }: { admin?: boolean }) {
  return (
    <main className={admin ? styles.adminPage : styles.page} aria-label="화면 불러오는 중" aria-busy="true">
      {admin && <aside className={styles.sidebar}><span className={styles.brandMark}><img src="/logo.png" alt="" width="36" height="36" /><span className={styles.logo} /></span>{Array.from({ length: 5 }, (_, index) => <span className={styles.nav} key={index} />)}</aside>}
      <section className={styles.main}>
        <header className={styles.header}><span className={styles.brandMark}><img src="/logo.png" alt="" width="36" height="36" /><span className={styles.logo} /></span><span className={styles.short} /></header>
